...
> **Note:** The "GREEN — Autonomous" banner shown above is NOT generated by Milton server code.
> This text appears in LLM responses when certain UI clients (Open WebUI, phone listener UI) include
> it in their system prompts. It is a UI/client-side indicator, not a Milton gateway or server mode.
> There is no configuration in Milton to disable or rename this banner as it originates from the LLM's
> response based on the client's prompt template.
